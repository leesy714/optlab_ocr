...
def test(classes=None):
    ...
    for imgs, crafts, ys, groups, boxing, idx in test_loader:
        ...
        print("[file-{:06d}] label f1-score: {:.4}, group f1-score: {:.4}".format(
            file_num, float(f1_label), float(f1_group)))
        ...
        #np.save("./res/model/{:06d}_craft.npy".format(file_num), craft)
        #np.save("./res/model/{:06d}_true.npy".format(file_num),  ys)
        #np.save("./res/model/{:06d}_pred.npy".format(file_num), pred)
