...
class Pipeline:
    ...
    def save_img(self, img, img_num):
        origin_img_path = os.path.join(self.res_path, "origin_noise_img")
        if not os.path.exists(origin_img_path):
            os.makedirs(origin_img_path)
        cv2.imwrite(os.path.join(origin_img_path, "img_"+str(img_num)+".png"), img)
        
    def transform(self, imgs, ys, batch_num):
        assert imgs.shape[0] == ys.shape[0]
        batch = imgs.shape[0]
        img_num = batch*batch_num
        for b in range(batch):

            imgs[b] = self.noise_generate(imgs[b].copy())
            # noise generate한 결과 img로 따로 저장
            self.save_img(imgs[b], img_num)
            print(img_num)
            img_num += 1
    ...
    def run(self):
        ...
        for batch in tqdm.tqdm(self.load_files()):
            ...
            imgs, ys = self.transform(imgs, ys, cnt)
