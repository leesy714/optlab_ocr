...
class RandomText:
    ...
    def save_img(self, img, img_num):
        origin_img_path = os.path.join(self.res_path, "origin_img")
        if not os.path.exists(origin_img_path):
            os.makedirs(origin_img_path)
        img.save(os.path.join(origin_img_path, "img_"+str(img_num)+".png"))
...
if __name__=='__main__':
    ...
    random_text = RandomText("template/건강보험자격득실확인서 포맷.png", text_type_lists, location_lists,
                             fontsize_lists, output_num=200, batch_size=10)
    img = random_text.run()
